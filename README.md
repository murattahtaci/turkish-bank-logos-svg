# Türkiye banka logoları — SVG

**Turkish bank logos in SVG** · [English below](#english)

Türkiye'deki 36 banka ve e-para kuruluşunun **orijinal** logoları, SVG
biçiminde. Hiçbiri elle çizilmedi ya da yeniden yorumlanmadı: her dosya aşağıda
yazan kaynaktan alındı, render edilip gözle doğrulandı ve web'de doğrudan
kullanılacak şekilde normalleştirildi.
...
ayırır. Sınama 47 durumla yapılıyor: 42 gerçek banka adı biçimi ve eşleşmemesi
gereken 5 durum ([`test/`](test/index.test.mjs)).

npm ile: `npm install github:murattahtaci/turkish-bank-logos-svg`

### Liste

Kurum adları, dosya yolları, boyutlar (`viewBox`), kaynak ve lisans makine
tarafından okunabilir biçimde [`logos.json`](logos.json) içinde.
...
| `freedombank.svg` | Freedom Bank (eski Turkish Bank) | kurumun marka dosyası — *bkz. not* | Kurumun marka varlığı |
...
- **Freedom Bank:** elimize geçen dosya koyu zemin içindi ("FREEDOM" yazısı
  beyaz). Beyaz zeminde görünsün diye yazının rengi markanın kendi paletindeki
  `#154734` koyu yeşiline çevrildi. Kalkan değiştirilmedi.
...
**Tosla** ve **Getir Finans** için SVG bulunamadı: kendi siteleri raster (PNG/JPG)
logo basıyor. Kurumun kendi sitesinden ya da marka kitinden bir SVG
bulursanız pull request açabilirsiniz — kaynağını tabloya ekleyerek.

Yeni bir logo eklerken: dosya `svg/` altına, kaydı `logos.json`'a, satırı bu
tabloya; banka adının eşleşmesi gerekiyorsa `index.js` içindeki `MATCH`'e. Sonra:
...
Original SVG logos of **36 banks and e-money institutions in Turkey**, collected
...
- **Not redrawn.** Each file comes from the source in the table above (Wikimedia
  ...
  was rendered and checked by eye, and normalized for the web.
...
- **Licenses per file:** 20 public domain (Wikimedia Commons), 1 CC BY-SA 4.0
  (A&T Bank — attribute and share alike), 15 trademark assets of the institution.
- **Missing:** Tosla, Getir Finans — pull requests welcome.
